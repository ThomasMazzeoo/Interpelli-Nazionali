...
# URL temporaneo per i test (aggiornalo a 2026-2027 quando lo pubblicheranno)
URL_BASE = "https://www.istruzioneimperia.gov.it/pagine/interpelli-im---as-2024-2025-1"

def run(url_visti):
    nuovi_interpelli = []
    headers = {'User-Agent': 'Mozilla/5.0'}
    print(f"📡 [LIGURIA] Connessione a Imperia...")
    ...
    try:
        ...
        start_row = 1
        # Il bot legge le righe finché non trova l'intestazione giusta (saltando le leggi e i titoli)
        for i, riga in enumerate(righe):
            ...
            if 'cdc' in text and 'denominazione' in text:
                ...
                for j, s in enumerate(intestazioni):
                    ...
                    elif s_clean == 'dal': idx_dal = j
                    elif s_clean == 'al': idx_al = j
        ...
        for riga in righe[start_row:]:
            ...
            # ID DETERMINISTICO (Evita i duplicati)
            ...
            # Qui il bot prende la colonna esatta a prescindere dalle celle vuote alla fine!
            ...
            # Il nostro Super Radar per le date!
            ...
            if link_tag:
                ...
                if not url_avviso.startswith('http') and not url_avviso.startswith('mailto:'):
                    url_avviso = "https://www.istruzioneimperia.gov.it" + url_avviso
