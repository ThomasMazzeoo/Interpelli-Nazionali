...
def run(url_visti):
    risultati = []
    print("    Inizio scraping Provincia di Oristano (USR Sardegna - Interpelli)...")

    url_base = "https://www.mim.gov.it/web/oristano/notizie"
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"    ❌ Errore USR Oristano, status code: {resp.status_code}")
            return risultati
        ...
        for articolo in articoli[:20]:
            ...
            # Siccome siamo nella pagina /notizie, filtriamo solo le news che riguardano gli interpelli
            if "interpell" not in titolo.lower():
                continue
        ...
        print(f"    ❌ Errore durante lo scraping di Oristano: {e}")
