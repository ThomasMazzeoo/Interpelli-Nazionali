...
def run(url_visti):
    risultati = []
    print("    Inizio scraping Provincia di Pistoia (USR Toscana - Interpelli)...")

    url_base = "https://www.ufficioscolasticoprovinciale.pistoia.it/category/docenti/interpelli-docenti/"
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"    ⚠️ Errore USR Pistoia, status code: {resp.status_code}")
            return risultati

        soup = BeautifulSoup(resp.text, 'html.parser')

        articoli = soup.find_all('article', class_='scheda-news')
        ...
        for articolo in articoli[:20]:
            ...
            data_raw = ""
            div_data = articolo.find('div', class_='scheda-icona-small')
            if div_data:
                data_raw = div_data.get_text(strip=True)
        ...
        print(f"    ⚠️ Errore durante lo scraping di Pistoia: {e}")
