...
# L'URL ufficiale della provincia di Padova
URL_BASE = "https://padova.istruzioneveneto.gov.it/category/interpelli-personale-docente/"

def estrai_data_scadenza_mirata(testo):
    """
    Cerca una data SOLO se è vicina a parole chiave DI SCADENZA ESATTE.
    Integra logica temporale per evitare gli anni '90.
    """
    testo = testo.lower().replace('\n', ' ')
    
    # Radar chirurgico per le date di scadenza
...
def run(url_visti):
    nuovi_interpelli = []
    headers = {'User-Agent': 'Mozilla/5.0'}
    print("📡 [VENETO] Connessione a Padova...")
    ...
    try:
        ...
        for art in articoli[:20]:
            ...
            # Filtro contro avvisi non pertinenti
            if any(x in titolo_raw.lower() for x in ['decreto', 'graduatorie', 'gps', 'ata', 'mobilità', 'assunzioni', 'errata corrige']): 
                continue

            # Esploratore Web per leggere le Classi di Concorso
            dettagli = esplora_dettaglio(url_avviso)
            cdc_totali = list(set(estrai_cdc(titolo_raw) + dettagli["cdc_extra"]))
            
            data_pulita = ""
            
            # FASE 1: Ricerca della scadenza nella pagina web HTML dell'interpello
            try:
            ...
            # FASE 2: Entrata del Bot nei PDF allegati se l'HTML non ha la data
        ...
        print(f"  ❌ Errore critico su Padova: {e}")
